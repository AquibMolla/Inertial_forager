...
if st.button("Run Simulation"):
    ...
    anim = FuncAnimation(
        ...
        lambda frame: [
            ...
            [ax1.plot(fy, fx, 'go', markersize=5, alpha=0.7, label='Food') for fx, fy in [zip(*food_history[frame])] if food_history[frame] else None,
            ...
            [ax1.text(0.5, 0.7, 'Forager died!', ha='center', va='center', transform=ax1.transAxes, color='red', fontsize=20) if frame == len(path)-1 else None]
        ],
        frames=frames,
        init_func=lambda: [
            ax1.clear(), ax2.clear(),
